...
@Component({
    selector: 'pp-diff-tree',
    templateUrl: './diff-tree.component.html',
    styleUrls: ['./call-tree.component.css'],
})
export class DiffTreeComponent implements OnInit, OnChanges, AfterViewInit {
    @HostBinding('class') hostClass = 'l-calltree';
    @Input() canSelectRow: boolean;
    @Input() rowSelection: string;
    @Input() selectedRowId: string;
    ...
    set callTreeData(callTreeData: ITransactionDetailData) {
        this.originalData = callTreeData;
        let diffRatio = this.calcTimeRatio(callTreeData.callStack[0][callTreeData.callStackIndex.begin], callTreeData.callStack[0][callTreeData.callStackIndex.end]);
        let diffData = this.makeGridData(callTreeData.callStack, callTreeData.callStackIndex, diffRatio);
        this.rowData = diffData;
    }
    ...
    private makeGridData(callTreeData: any, oIndex: any, ratio: number): IGridData[] {
        const newData = [];
        const parentRef = {};
        ...
        for (let i = 0; i < callTreeData.length; i++) {
            ...
            parentRef[callTree[oIndex.id]] = oRow;
            this.makeRow(callTree, oIndex, oRow, i, ratio);
    ...
    private makeRow(callTree: any, oIndex: any, oRow: IGridData, index: number, ratio: number): void {
        oRow['index'] = index;
        oRow['id'] = callTree[oIndex.id];
        oRow['method'] = callTree[oIndex.title];
        oRow['argument'] = callTree[oIndex.arguments];
        ...
        oRow['gap'] = callTree[oIndex.gap];
        oRow['exec'] = callTree[oIndex.elapsedTime];
        oRow['execPer'] =  callTree[oIndex.elapsedTime] ? Math.ceil((callTree[oIndex.end] - callTree[oIndex.begin]) * ratio) : '';
        oRow['selp'] = callTree[oIndex.executionMilliseconds];
        oRow['selpPer'] = callTree[oIndex.elapsedTime] && callTree[oIndex.executionMilliseconds] ?
            ( Math.floor( callTree[oIndex.executionMilliseconds].replace(/,/gi, '') ) / Math.floor( callTree[oIndex.elapsedTime].replace(/,/gi, '') ) ) * 100
            : 0;
